...
| `superAdmin` | 不公开（本地由 `TenonAdmin:Seed:AdminPassword` 决定） | 不受限 | 214 | 全部模块，全部按钮 |
...
三个账号在客户接口上只有读权限，页面上根本不渲染增删改按钮。总部管理员额外拿到了内核自带的整套系统管理菜单，走的是正常角色授权而非超管绕过——这个仓库要展示的正是这条路径，所以 `superAdmin` 不参与在线演示：它绕过 `[RolePermission]`，密码公开就等于把超管交给所有人。登录页对上面三个账号有一键登录按钮，不用手打密码。
...
公开部署另外打开 `CrmDemo:ReadOnly`（`DemoReadOnlyFilter`）：除登录和导入外，一切非 GET 返回 `403` / `41002`。菜单、按钮、表单照常渲染，点得动，只是写不进库——**上面三个试用账号的密码是公开的，线上就是靠这道闸门而不是靠密码**。别用内核的 `TenonAdmin:DemoMode` 代替它：那个没有放行名单，会把导入 POST 一起拦掉。

本地默认 `ReadOnly=false`，四个账号（含超管）的增删改查都是真的；再把 `ImportDryRun` 设为 `false`，导入也会真的落库。
