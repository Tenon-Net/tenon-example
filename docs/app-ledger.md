...
- App: `/root/opt/tenon/tenon-example`, a plain `git clone` of this repo's `dev` branch, `docker compose -p tenon-example up -d --build`. Secrets live in `.env` and the demo policy in `docker-compose.override.yml` (both untracked, server-local) — deliberately excluded from git the same way the old kernel demo's deployment was. The override pins `TenonAdmin__DemoMode=false`, `CrmDemo__ReadOnly=true` and `CrmDemo__ImportDryRun=true`. DemoMode has to stay **off** because it blocks the import wizard's POSTs (see [`v0.5.0-upgrade.md`](v0.5.0-upgrade.md)); `ReadOnly` is the app's own gate (`DemoReadOnlyFilter`) that replaces it — same 403/41002, but login and `/api/v1/biz/customer/import` are allowlisted. **That gate is what keeps the public demo safe, not the credentials**: the three trial passwords are published in the README, and `总部管理员` genuinely holds 60 non-GET grants across system management (that is the point of the account). `superAdmin` is deliberately kept out of the public demo — it bypasses `[RolePermission]`, so its password is rotated and unpublished; the live one was rotated on 2026-08-16. `ReadOnly` and `ImportDryRun` must both stay on — with only the first, the import wizard writes for real.
